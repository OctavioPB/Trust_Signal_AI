...
from storage.object_store import BUCKET_RAW_AUDIO, BUCKET_RESUMES, _strip_protocol

logger = structlog.get_logger(__name__)

DEFAULT_RETENTION_DAYS = 90   # CLAUDE.md §8 rule 7 — audio and resumes
...
def apply_resumes_lifecycle(
    ...
    """Apply a 90-day expiry lifecycle rule to the resumes bucket.

    Implements CLAUDE.md Hard Rule #7 — resume files are PII-adjacent and
    must be deleted within 90 days unless the customer has opted into extended
    retention. Safe to call repeatedly (idempotent MinIO PUT).
    ...
    lifecycle = LifecycleConfig(
        [
            Rule(
                ...
                rule_id="resumes-auto-expire",
                expiration=Expiration(days=retention_days),
            )
        ]
    )

    client.set_bucket_lifecycle(BUCKET_RESUMES, lifecycle)
