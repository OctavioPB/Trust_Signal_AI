...
_CANDIDATE_UUID = "cand-prescreening-uuid"
...
_PYTHON_CODE_B64 = base64.b64encode(_PYTHON_CODE).decode()

_README_TEXT = "Python microservices platform. FastAPI, Docker, Redis."


def _make_perp_scorer(loss: float = 1.0, n_tokens: int = 20):
    """Return a (tokenizer, model) stub yielding a fixed perplexity loss."""
    tok = MagicMock()
...
def _make_embedding_model(similarity: float = 0.9):
    """Return a sentence-transformer stub with fixed similarity."""
    dim = 8
    a = np.ones(dim) / np.sqrt(dim)
    b_angle = np.arccos(similarity) if abs(similarity) <= 1 else 0.0
    b = np.array([np.cos(b_angle)] + [np.sin(b_angle) / np.sqrt(dim - 1)] * (dim - 1))
    b /= np.linalg.norm(b)

    model = MagicMock()
    calls = {"n": 0}

    def encode_side(text, *a, **kw):
        c = calls["n"] % 2
        calls["n"] += 1
...
def test_resume_text_produces_valid_resume_score():
    ...
    tok, mod = _make_perp_scorer(loss=2.0)
    emb_model = _make_embedding_model(similarity=0.5)

    with patch("ml.features.resume_perplexity.AutoModelForMaskedLM", return_value=mod), \
         patch("ml.features.resume_perplexity.AutoTokenizer", return_value=tok), \
         patch("ml.features.section_uniformity.SentenceTransformer", return_value=emb_model):
        ...
        sections = {"Skills": "Python FastAPI", "Experience": "Built services"}
        perp   = score_resume_perplexity(_CANDIDATE_UUID, sections)
        burst  = score_resume_burstiness(_CANDIDATE_UUID, sections)
        vocab  = score_vocab_richness(_CANDIDATE_UUID, _RESUME_TEXT)
        unif   = score_section_uniformity(_CANDIDATE_UUID, sections)
...
def test_mock_repo_produces_valid_repo_score():
    ...
    tok, mod = _make_perp_scorer(loss=1.5)
    files = [("main.py", _PYTHON_CODE.decode())]

    with patch("ml.features.code_perplexity.AutoModelForMaskedLM", return_value=mod), \
         patch("ml.features.code_perplexity.AutoTokenizer", return_value=tok):

        from ml.features.code_perplexity import CodePerplexityScorer
        ppl_scorer     = CodePerplexityScorer()
...
def test_prescreening_with_all_three_signals():
    ...
    result = engine.compute(
        ...
        repo_ai_score=75.0,
        interview_trust_score=25.0,   # low trust → high suspicion
    )
...
def test_cross_correlation_coherent_candidate():
    ...
    emb_model = _make_embedding_model(similarity=0.95)
...
def test_cross_correlation_incoherent_candidate():
    ...
    emb_model = _make_embedding_model(similarity=0.0)
    scorer    = CrossCorrelationScorer(_model=emb_model)
    result    = scorer.score(
        candidate_uuid=_CANDIDATE_UUID,
...
def test_profile_producer_payload_has_no_pii():
    ...
    # Must contain UUID-based identifiers
    assert "candidate_uuid" in payload
    assert uuid.UUID(payload["candidate_uuid"])   # valid UUID
...
def test_prescreening_store_path_uses_table_name():
    ...
    from storage.prescreening_store import PreScreeningStore, TABLE_PRESCREENING
    import time
    ...
    from storage.prescreening_store import PreScreeningRecord
    record = PreScreeningRecord(
        candidate_uuid="u", prescreening_score=0.0, resume_ai_score=0.0,
        repo_ai_score=None, interview_trust_score=None, suspicion_index=0.0,
        flagged=False, severity="low", flag_reason="", signals_json="[]",
        scored_at=time.time(),
    )
    store.append_record(record)
